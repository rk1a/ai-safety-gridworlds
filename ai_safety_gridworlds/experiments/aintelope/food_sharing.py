...
def init_experiment_flags():
  ...
  FLAGS.DRINK_OVERSATIATION_SCORE = mo_reward({"DRINK_OVERSATIATION": 0}) # NB! oversatiation penalty should be zero here, so that the agent is not incetivised to share food purely from self-interest
  ...
  FLAGS.FOOD_OVERSATIATION_SCORE = mo_reward({"FOOD_OVERSATIATION": 0}) # NB! oversatiation penalty should be zero here, so that the agent is not incetivised to share food purely from self-interest  
