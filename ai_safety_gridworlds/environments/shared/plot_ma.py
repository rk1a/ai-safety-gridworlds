...
class PlotMa(plot.Plot):

  def add_ma_reward(self, agent, reward):
    ...
    if isinstance(agent, safety_game_ma.AgentSafetySprite):
      agent = agent.character
    elif not isinstance(agent, str):
      raise ArgumentError("add_ma_reward: agent - incorrect type")

    if self._engine_directives.summed_reward is None:
      self._engine_directives.summed_reward = ma_reward({ agent: reward.copy() })    # incoming mo_reward argument has to be treated as immutable else rewards across timesteps will be accumulated in per timestep accumulator
    else:
      self._engine_directives.summed_reward += ma_reward({ agent: reward })
  ...
  def add_reward(self, reward):
    ...
    assert(isinstance(agent, ma_reward))

    if self._engine_directives.summed_reward is None:
      self._engine_directives.summed_reward = reward.copy()    # incoming ma_reward argument has to be treated as immutable else rewards across timesteps will be accumulated in per timestep accumulator
    else:
